 ...
 * ParametrosUtghra
 * ----------------
 * Panel de parámetros específico para Hoteles y Gastronomía (UTGHRA–FEHGRA).
 *
 * Espera props:
 ...
 * - complementoServicio: boolean
 * - tipoAporteSindical: "ninguno" | "afiliado" | "noAfiliado"
 * - vacacionesOn: boolean
 * - vacDiasTrabajados: number
 * - vacDiasManual: number
 * - obraSocialBase: "48" | "48+extras+vac" | "real"
 */
...
export default function ParametrosUtghra({ values, onChange }) {
  ...
  return (
    <div className="space-y-3 rounded-2xl border border-slate-200 bg-white/80 p-4">
      <h2 className="text-base font-semibold text-slate-800">
        Parámetros · Hoteles y Gastronomía (UTGHRA–FEHGRA)
      </h2>
      ...
      <div className="mt-3 rounded-xl border border-sky-200 bg-sky-50/70 p-3 space-y-2">
        ...
        <label className="flex items-center gap-2 text-sm text-slate-800">
          <input
            ...
            checked={!!values.asistenciaPerfecta}
            onChange={(e) => onChange("asistenciaPerfecta", e.target.checked)}
          />
          Asistencia perfecta (10% sobre básico)
        </label>
        ...
        <label className="flex items-center gap-2 text-sm text-slate-800">
          <input
            type="checkbox"
            className="h-4 w-4 accent-sky-600"
            checked={!!values.complementoServicio}
            onChange={(e) => onChange("complementoServicio", e.target.checked)}
          />
          Complemento por servicio (12% sobre básico)
        </label>
        ...
        <div className="space-y-1 pt-1">
          ...
          <select
            className="w-full rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-sm"
            value={values.tipoAporteSindical || "ninguno"}
            onChange={(e) => onChange("tipoAporteSindical", e.target.value)}
          >
            <option value="ninguno">Sin aporte sindical</option>
            <option value="afiliado">Afiliado (2,5% cuota sindical)</option>
            <option value="noAfiliado">
              No afiliado (2% contribución solidaria)
            </option>
          </select>
        </div>
      </div>
      ...
      <div className="space-y-1 pt-1">
        ...
        <select
          className="w-full rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-sm"
          value={values.obraSocialBase || "real"}
          onChange={(e) => onChange("obraSocialBase", e.target.value)}
        >
          <option value="48">Solo base 48 (rem + NR escala)</option>
          <option value="48+extras+vac">
            Base 48 + horas extras + vacaciones
          </option>
          <option value="real">
            Total remunerativo real del mes
          </option>
        </select>
      </div>
    </div>
