 ...
 *
 * Columnas aceptadas: [zona,] categoria, basico, no_remunerativo
 * La zona se detecta por el encabezado, y si no hay encabezado reconocible se
 * infiere por la cantidad de columnas.
 *
...
export function parsearCsvEscala(texto) {
  ...
  lineas.slice(1).forEach((linea, idx) => {
    ...
    // Si el encabezado no lo dice, se infiere por cantidad de columnas.
    const conZona = headerDeclaraZona || col.length >= 4;
    const zona = conZona ? col[0] : null;
    const categoria = conZona ? col[1] : col[0];
    const textoBasico = conZona ? col[2] : col[1];
    const textoNr = conZona ? col[3] : col[2];
...
export function generarCsvEscala(claves = [], sueldos = {}) {
  ...
  const cabecera = conZona ? "zona,categoria,basico,no_remunerativo" : "categoria,basico,no_remunerativo";
