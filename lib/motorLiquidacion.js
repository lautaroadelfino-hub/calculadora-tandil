...
export function procesarRecibo(convenio, datosEscala, valoresUsuario, paramsGanancias = null, opciones = {}) {
  ...
  if (reglaAntiguedad && valoresUsuario.antiguedad_años > 0) {
    ...
    pctAnt = porcentajeAntiguedad(reglaAntiguedad, valoresUsuario.antiguedad_años);
  }
  ...
  if (convenio.reglas_calculo.adicionales_por_unidad) {
    for (const [id_unidad, regla] of Object.entries(convenio.reglas_calculo.adicionales_por_unidad)) {
      ...
      const monto = (Number(valoresDelPeriodo[regla.valor]) || 0) * cantidad;
      if (monto <= 0) continue;
      ...
      if (regla.naturaleza === "remunerativo") {
        porUnidadRem += monto;
        lineasRecibo.push({ concepto: etiqueta, tipo: "remunerativo", monto, porUnidad: true });
      } else if (regla.con_incidencia === true) {
        porUnidadNRConIncidencia += monto;
        lineasNRPorUnidad.push({ concepto: etiqueta, tipo: "no_remunerativo", monto, porUnidad: true });
      } else {
        porUnidadNRSinIncidencia += monto;
        lineasNRPorUnidad.push({ concepto: etiqueta, tipo: "no_remunerativo", monto, porUnidad: true, sinIncidencia: true });
      }
    }
  }
  ...
  if (pctAnt > 0 && antiguedadDiferida) {
    ...
    if (antiguedad > 0) {
      ...
      lineasRecibo.splice(1, 0, { concepto: "Antigüedad", tipo: "remunerativo", monto: antiguedad });
    }
  }
  ...
  for (const aporte of APORTES_PERSONALES) {
    ...
    lineasRecibo.push({ concepto: aporte.label, tipo: "retencion", monto, rubro: aporte.rubro });
