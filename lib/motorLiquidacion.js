...
export function procesarRecibo(convenio, datosEscala, valoresUsuario, paramsGanancias = null, opciones = {}) {
  ...
  const cargaHoraria = valoresUsuario.carga_horaria ? Number(valoresUsuario.carga_horaria) : horasCompletas;
  ...
  const baseOS = baseOsSinProrratear + extrasVariables;

  // --- 7. RETENCIONES LEY NACIONAL ---
  let totalRetenciones = 0;

  // Cada retención dice a qué rubro del Decreto 407/2026 va, para la composición
  // del costo laboral que el recibo muestra al final (lo que pone cada lado).
  const retencionesNacionales = [
    { nombre: "Jubilación (11%)", porcentaje: 0.11, rubro: "seguridad_social" },
    { nombre: "Ley 19.032 PAMI (3%)", porcentaje: 0.03, rubro: "inssjp" }
  ];

  retencionesNacionales.forEach(ret => {
    let monto = REM_REAL * ret.porcentaje;
    lineasRecibo.push({ concepto: ret.nombre, tipo: "retencion", monto: monto, rubro: ret.rubro });
    totalRetenciones += monto;
  });

  let montoOS = baseOS * 0.03;
  lineasRecibo.push({ concepto: "Obra Social (3%)", tipo: "retencion", monto: montoOS, rubro: "obra_social" });
  ...
  if (convenio.reglas_calculo.retenciones_sindicales) {
    ...
    for (const [id_retencion, regla] of Object.entries(retenciones)) {
      ...
      if (monto > 0) {
        ...
        lineasRecibo.push({ concepto: nombreConcepto, tipo: "retencion", monto: monto, rubro: "sindical" });
        totalRetenciones += monto;
      }
    }
  }
  ...
  // Decidido con el dueño: el SAC paga contribuciones. Es un interruptor de la
  // tabla para poder volver atrás desde /admin, sin desplegar.
  ...
  const artAlicuota = artInformada
    ? (Number(artDelUsuario) || 0) / 100
    : reglaArt && reglaArt.alicuota_tipica != null
      ? Number(reglaArt.alicuota_tipica)
      : null;
  ...
  return {
    ...
    costoEmpleador: contribuciones.regimen
      ? {
          ...
          costoPorDia: costoLaboral / POR_DEFECTO.diasDelMesParaCostoDiario,
          // Los siete rubros del decreto, con lo que pone cada lado.
          rubros: resumenPorRubro(lineasRecibo).rubros,
    ...
    metodo: {
      ...
      artAlicuota,
      artLaInformoLaPersona: artInformada,
