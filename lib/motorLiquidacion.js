...
export function procesarRecibo(convenio, datosEscala, valoresUsuario, paramsGanancias = null, opciones = {}) {
  ...
  if (cargaHorariaPedida === null || !(cargaHorariaPedida > 0)) {
    if (valoresUsuario.carga_horaria !== undefined) {
      avisos.push(
        `No cargaste las horas semanales: se calculó por jornada completa, ${horasCompletas} hs.`
      );
    }
  } else if (cargaHorariaPedida > horasCompletas) {
    avisos.push(
      `Cargaste ${cargaHorariaPedida} hs semanales, más que la jornada completa de ${horasCompletas}: el básico va por jornada completa y el resto como horas extras.`
    );
  } else {
  ...
  if (!reglaAntiguedad && Number(valoresUsuario.antiguedad_años) > 0) {
    avisos.push(
      "Este convenio no usa la antigüedad: los años que cargaste no cambian el resultado."
    );
  }
  ...
  if (convenio.reglas_calculo.retenciones_sindicales) {
    ...
    for (const [id_retencion, regla] of Object.entries(retenciones)) {
      ...
      if (monto > 0) {
        ...
        // El rubro de la composición del costo laboral: sindical salvo que la
        // regla diga otro (el aporte fijo a la obra social es obra social).
        const rubroRetencion = regla.rubro || "sindical";
        if (!RUBROS_DEL_COSTO_LABORAL.some((r) => r.id === rubroRetencion)) {
          throw new Error(
            `La retención "${nombreConcepto}" tiene el rubro "${regla.rubro}", que el motor no conoce. Revisala en el panel de administración.`
          );
        }
        lineasRecibo.push({ concepto: nombreConcepto, tipo: "retencion", monto: monto, rubro: rubroRetencion, detalle });
        totalRetenciones += monto;
      }
    }
  }
