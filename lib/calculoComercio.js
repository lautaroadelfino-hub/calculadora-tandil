// lib/calculoComercio.v2.js
// Motor declarativo – Comercio (CCT 130/75)
// Entra el mismo "entry" y opciones que ya usás.

export function calcularComercioV2({
  entry,
  aniosAntiguedad = 0,
  ...
  divisorHoras = 200,      // divisor base 48 hs (recomendado 200)
  ...
  // ---------- prorrateos remunerativos base (reales)
  ...
  // ---------- BASES declaradas (una sola fuente de verdad)
  // 1) REM reales (para Jubilación y PAMI)
  // 2) TOTAL real (para Sindicato/FAECyS)
  const bases = {
    REM_REAL: totalRemunerativo,
    TOTAL_REAL: totalRemunerativo + totalNoRemunerativo,
    // 3) OS base 48 (NO depende de factorCH ni de extras)
    OS_48: (() => {
      ...
      return basico48 + antig48 + titulo48 + funcion48 + pres48; // sin horas ni vacaciones
    })(),
  };

  // ---------- DEDUCCIONES declarativas
  const itemsDeduccion = [
    ...
    { label: "Obra social (3%)",       monto: bases.OS_48       * 0.03, base: bases.OS_48, baseTag: "Base 48 hs" },
  ...
  return {
    // desagregados REM
    basico, antiguedad, presentismo, adicionalTitulo, adicionalFuncion,
    horasExtras50, horasExtras100, valorHora, horasMensuales,
