...
export function calcularComercio({
  ...
  noRemunerativo = 0,      // $ otros no rem / acuerdos
  ...
  vacDiasTrabajados = 0,          // si antigüedad < 6 meses: días trabajados (1 c/20)
  vacDiasManual = 0,              // override de días (si > 0, pisa el cálculo automático)
}) {
  // ——— Básico prorrateado por carga horaria
  ...
  // No remunerativo fijo de la escala
  const noRemFijo = Number(entry?.noRem || 0);

  // ——— Antigüedad (1% anual) y Presentismo comercio: (básico + antigüedad) / 12
  ...
  // ——— Horas extras (NO integran base “normal y habitual” de vacaciones)
  ...
  // ——— Remunerativos del mes (SIN vacaciones todavía)
  ...
  // ——— Vacaciones: días según antigüedad / manual / <6 meses (1 c/20)
  ...
  return {
    ...
    // vacaciones como PLUS (no sueldo duplicado)
    vacacionesPlus: +vacacionesPlus.toFixed(2),
    vacacionesDias: +vacacionesDias,
    valorDiaBase30: +(baseVacNH / 30).toFixed(4),
    valorDiaBase25: +(baseVacNH / 25).toFixed(4),
    plusPorDia: +(baseVacNH / 150).toFixed(4),
