...
export function calculoComercio({
  entry,
  aniosAntiguedad = 0,
  // En Comercio, el convenio no prevé estos adicionales. Se dejan por compat
  // pero hoy no afectan el cálculo.
  titulo = "ninguno",
  funcion = 0,
  horas50 = 0,
  horas100 = 0,
  ...
  // En Comercio el convenio no prevé adicionales por título ni por función.
  // Se dejan explícitamente en 0 para evitar arrastre del esquema público.
  const adicionalTitulo = 0;
  const adicionalFuncion = 0;

  const presentismo = (basico + antiguedad) / 12;

  // ---- horas extra
  const horasMensuales =
    (num(divisorHoras) > 0 ? num(divisorHoras) : 200) * factorCH;
  const baseHora =
  ...
  // Base para vacaciones (nocturnidad/horario NO suman acá)
  const baseVacNH = basico + antiguedad + presentismo;

  // ➕ NUEVO: valores por día que la UI necesita
  const valorDiaBase25 = baseVacNH / 25;
  const valorDiaBase30 = baseVacNH / 30;
  const plusPorDia = baseVacNH / 150;

  // Plus total por vacaciones
  ...
  const totalNoRemunerativo =
    baseNR + antiguedadNR + presentismoNR;
  ...
  const bases = {
    ...
    // Dentro de: const bases = { ... OS_48: (() => { ... })(), }
    OS_48: (() => {
      // ==== REM 48 (puro)
      const antig48 = basico48 * 0.01 * num(aniosAntiguedad);

      // En Comercio no se aplican adicionales de título ni función a 48 hs.
      const titulo48 = 0;
      const funcion48 = 0;

      const pres48 = (basico48 + antig48) / 12;
      const rem48_total = basico48 + antig48 + pres48;

      // ==== NR 48 (de escala) + derivados NR a 48 hs
      const nr48 = nrEscala48; // tu monto NR de escala definido a 48 hs
      const antigNR48 = nr48 * 0.01 * num(aniosAntiguedad);
      const presNR48 = (nr48 + antigNR48) / 12;
      const nr48_total = nr48 + antigNR48 + presNR48;

      // ==== Base 48 completa para OS (sin extras ni vacaciones)
  ...
  const itemsDeduccion = [
    {
      label: "Jubilación (11%)",
      monto: bases.REM_REAL * 0.11,
      base: bases.REM_REAL,
    },
    {
      label: "Ley 19.032 – PAMI (3%)",
      monto: bases.REM_REAL * 0.03,
      base: bases.REM_REAL,
    },
    {
      label: "Obra social (3%)",
    ...
    {
      label: "Sindicato (2%)",
      monto: bases.TOTAL_REAL * 0.02,
      base: bases.TOTAL_REAL,
    },
    {
      label: "FAECyS (0,5%)",
      monto: bases.TOTAL_REAL * 0.005,
      base: bases.TOTAL_REAL,
    },
  ...
  if (otros)
    itemsDeduccion.push({ label: "Otros descuentos", monto: otros });

  const totalDeducciones = itemsDeduccion.reduce(
    (acc, it) => acc + num(it.monto || 0),
    0
  );
  const liquido =
  ...
  return {
    ...
    // Bases (para UI / debug)
    bases, // { REM_REAL, TOTAL_REAL, OS_48 }
    baseOS, // base final usada para OS
