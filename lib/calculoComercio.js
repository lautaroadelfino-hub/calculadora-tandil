...
export function calcularComercio({
  ...
  // <<< NUEVO: vacaciones
  vacacionesDias = 0,          // cantidad de días a liquidar
  vacacionesBrutoRef = 0,      // bruto de referencia (rem + no rem) del mes anterior
}) {
  ...
  // Vacaciones (base 25) – REMUNERATIVO
  const vacDias = Math.max(0, Number(vacacionesDias) || 0);
  const vacBrutoRef = Math.max(0, Number(vacacionesBrutoRef) || 0);
  const vacacionesMonto = vacDias > 0 && vacBrutoRef > 0 ? (vacBrutoRef / 25) * vacDias : 0;
  ...
  return {
    ...
    vacacionesMonto: +vacacionesMonto.toFixed(2), // <<< NUEVO
