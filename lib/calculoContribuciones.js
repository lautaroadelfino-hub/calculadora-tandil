...
const NOMBRE_DE_BASE = {
  ...
  remunerativo_mas_no_remunerativo: "Remunerativo + no remunerativo",
};
...
export function calcularContribuciones({
  ...
  if (!tabla) {
    avisos.push(
      "No hay tabla de contribuciones patronales cargada para este período, así que el recibo no " +
        "puede mostrar la sección del empleador que exige la ley desde el 01/06/2026. " +
        "Cargala en /admin → Contribuciones."
    );
    return { lineas: [], total: 0, regimen: null, detraccion: null, avisos };
  }

  const bases = { remunerativo: Number(remunerativo) || 0, noRemunerativo: Number(noRemunerativo) || 0 };
  ...
  if (tabla.detraccion && tabla.detraccion.monto != null) {
    ...
    avisos.push(
      "La tabla de contribuciones no trae la detracción de la Ley 27.541: las contribuciones se " +
        "calcularon sin restarla, así que salen más altas de lo que corresponde."
    );
  }
  ...
  if (art) {
    ...
    if (!tieneAlicuota) {
      avisos.push(
        "No se informó la alícuota de ART, así que esa fila quedó sin monto y el costo del empleador " +
          "está incompleto. Cada empleador tiene la suya; el convenio puede tener cargada una típica en /admin."
      );
    }
    lineas.push({
 ...
 */
export function resumenPorRubro(detalle) {
  const rubros = {};
  ...
  for (const r of Object.values(rubros)) r.porcentaje = total > 0 ? (r.total / total) * 100 : 0;
