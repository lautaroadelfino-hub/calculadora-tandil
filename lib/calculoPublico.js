...
export function calcularPublico({
  ...
  // Divisor: Obras = hsSem*6 | Adm/SISP = hsSem*4.4
  const divisor =
    subRegimen === "obras"
      ? horasSem * 6
      : horasSem * 4.4;
  ...
  // 👇 ACÁ agregamos la base que usa Resultados.jsx para mostrar "Base: $ ..."
  const detalleDeducciones = [
    {
      label: "IPS (14%)",
      monto: descIPS,
      base: totalRemunerativo,
    },
    {
      label: "IOMA (4,8%)",
      monto: descIOMA,
  ...
  if (otros) {
    // Otros descuentos suelen ser importe fijo; si quisieras también podés poner base: totalRemunerativo
