...
export function normalizarEntradas(convenio, valores = {}, periodo = "") {
  ...
  for (const id of camposNumericos(convenio)) {
    ...
    if (crudo === "" || crudo === null || crudo === undefined) {
      if (id === "carga_horaria") {
        errores[id] = `Cargá las horas semanales que trabaja. La jornada completa de este convenio es de ${completas} hs.`;
    ...
    if (id === "carga_horaria") {
      if (n === 0) errores[id] = `Poné las horas semanales que trabaja (jornada completa: ${completas} hs). Con 0 no hay sueldo que calcular.`;
      else if (n > TOPES.horas_semanales) errores[id] = `Más de ${TOPES.horas_semanales} horas semanales no es posible.`;
      else if (n > completas) avisos.push(`Cargaste ${fmt(n)} hs semanales, más que la jornada completa de ${completas}: el básico se calcula por jornada completa y las horas de más se pagan como horas extras.`);
