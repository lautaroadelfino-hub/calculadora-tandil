...
export default function AdminPage() {
  ...
  // Parámetros de Impuesto a las Ganancias (colección aparte, nacional)
  const [gananciasPeriodoID, setGananciasPeriodoID] = useState("");
  ...
  // --- GESTIÓN DE PARÁMETROS DE GANANCIAS (Firestore: parametros_ganancias) ---
  const descargarPlantillaGanancias = () => {
  ...
  const cargarGananciasJSON = async (e) => {
    ...
    if (!gananciasPeriodoID || !/^\d{4}-\d{2}$/.test(gananciasPeriodoID)) {
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (!Array.isArray(datos.escala_anual) || !datos.deducciones_anuales) {
          throw new Error(
            "El archivo no tiene el formato correcto (faltan 'escala_anual' o 'deducciones_anuales')."
          );
        }

        const seguro = window.confirm(
          `¿Guardar las tablas de Ganancias para el período "${gananciasPeriodoID}"?`
        );
        if (!seguro) {
          e.target.value = null;
          return;
        ...
        await setDoc(doc(db, "parametros_ganancias", gananciasPeriodoID), datos);
        alert(`¡Tablas de Ganancias guardadas para ${gananciasPeriodoID}!`);
      } catch (error) {
  ...
  return (
    <div className="max-w-5xl mx-auto p-8 mt-10 bg-white rounded-xl shadow-lg border border-gray-200 mb-20">
      ...
      {/* --- GESTIÓN DE TABLAS DE GANANCIAS --- */}
      <div className="mb-6 p-5 bg-amber-50 border border-amber-200 rounded-xl">
        ...
        <p className="text-xs text-amber-800 mb-3">
          Cargá la escala del art. 94 y las deducciones cuando ARCA las actualice (enero y julio).
          Descargá la plantilla, reemplazá los montos con los oficiales, poné el ID del período y subila.
        </p>
        <div className="flex flex-col md:flex-row gap-4 items-center">
          <div className="w-full md:flex-1">
            ...
            <input
              type="text"
              value={gananciasPeriodoID}
              onChange={(e) => setGananciasPeriodoID(e.target.value)}
              placeholder="2026-01"
              className="w-full border border-amber-300 p-2.5 rounded-lg outline-none bg-white font-mono text-sm"
            />
          </div>
          <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto md:pt-5">
            <button type="button" onClick={descargarPlantillaGanancias} className="bg-white hover:bg-gray-100 text-amber-800 font-bold px-4 py-2.5 rounded-lg border border-amber-300 shadow-sm text-xs w-full sm:w-auto text-center transition-colors">
